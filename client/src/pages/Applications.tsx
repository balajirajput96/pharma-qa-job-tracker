...
export default function Applications() {
  ...
  const update = trpc.jobTracker.updateApplicationStatus.useMutation({
    onSuccess: async () => {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <section>
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">
          Keep your follow-ups visible
        </h1>
        <p className="mt-2 text-sm text-slate-600">
          Applications appear here when you create a draft. Update the status
          only after the real-world action happens.
        </p>
      </section>
      {rows.length === 0 ? (
        <Card className="border-dashed border-slate-300">
          <CardContent className="p-10 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Return to Ranked jobs and create an application draft for a
              suitable vacancy.
            </p>
          </CardContent>
        </Card>
              ...
              <CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {labels[row.application.status]}
                    </Badge>
                    <span className="text-xs text-slate-500">
                      CV: {row.application.cvVersion || "Not set"}
                    </span>
                  </div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    {row.company.name} <span className="mx-1">·</span>{" "}
