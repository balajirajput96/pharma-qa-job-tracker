...
const inputPath = resolve(
  process.argv.slice(2).find(argument => argument !== "--") ?? defaultInput
);
...
function publicValue(value: string | undefined) {
  ...
  return /^(not stated|not disclosed|not publicly listed|n\/a|na)$/i.test(
    normalized
  )
    ? ""
    : normalized;
...
function routeFor(value: string) {
  ...
  if (/direct vacancy|online application|direct apply/i.test(value))
...
const records: ScheduledVacancyInput[] = data
  .map(values => {
