...
import {
  ...
  recordGitHubPublishOutcome,
} from "../server/db.ts";

const projectRoot = resolve(import.meta.dirname, "..");
const date =
  process.env.REPORT_PUBLISH_DATE ?? new Date().toISOString().slice(0, 10);
...
try {
  ...
  if (dryRun) {
    ...
    await recordGitHubPublishOutcome(
      "dry_run",
      relativeReportPath,
      "Verified changed-file staging; commit and push skipped by dry-run mode."
    );
    console.log(
      JSON.stringify({
        status: "dry_run",
