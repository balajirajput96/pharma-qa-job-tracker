...
export const APPLICATION_SUBJECT =
  "IPQA Officer – 2 Years OSD Tablet Compression Experience";
export const DAILY_MONITORING_CRON = "0 0 3 * * *";
export const GUJARAT_PRIORITY_LOCATIONS = [
  "Vadodara",
  "Ahmedabad",
  "Halol",
  "Sanand",
...
export function canMarkDraftAsSent(
  approvalStatus: "draft" | "approved" | "sent" | "cancelled"
) {
...
function publicValue(value: string | undefined) {
  return Boolean(
    value && value.trim() && !/not publicly verified/i.test(value)
  );
}
...
export function locationPriority(location: string) {
  ...
  if (
    normalized.includes("bharuch") ||
    normalized.includes("savli") ||
    normalized.includes("dahej")
  )
    return 20;
...
export function inferVacancyLocation(title: string, currentLocation: string) {
  ...
  return (
    matches.find(([, expression]) => expression.test(text))?.[0] ??
    currentLocation
  );
}

export function inferVacancyRoute(
  title: string,
  currentRoute: "walk_in" | "direct" | "unverified"
) {
  if (currentRoute !== "unverified") return currentRoute;
  return /walk[-\s]?in|walkin/i.test(title) ? "walk_in" : currentRoute;
}

export function scoreVacancy(input: {
  title: string;
  eligibility: string;
  location: string;
  route: string;
  ...
  if (/tablet|compression|solid oral|\bosd\b|granulation|coating/.test(text))
    score += 20;
  if (
    /1\s*[-–to]+\s*3|2\s*[-–to]+\s*\d|\b2\s*years?\b/.test(text) ||
    input.twoYearHint
  )
    score += 10;
...
export type RankingProfile = Pick<
  ...
  "experienceYears" | "skills" | "preferredLocations"
>;
...
export function scoreVacancyForProfile(
  ...
  const locationMatch = preferredLocations.some(
    location =>
      location.length > 2 && input.location.toLowerCase().includes(location)
  );
  const normalizedSkills = profile.skills.toLowerCase();
  const roleText = `${input.title} ${input.eligibility}`.toLowerCase();
  const skillHits = ["ipqa", "tablet compression", "osd", "gmp"].filter(
    skill =>
      normalizedSkills.includes(skill) && roleText.includes(skill.split(" ")[0])
  ).length;
  const exactExperience =
    new RegExp(`\\b${profile.experienceYears}\\s*(?:years?|yrs?)\\b`).test(
      roleText
    ) ||
    new RegExp(
      `${Math.max(0, profile.experienceYears - 1)}\\s*[-–to]+\\s*${profile.experienceYears + 1}`
    ).test(roleText);
  return Math.min(
    100,
    baseScore +
...
async function upsertCompanyFromResearch(
  ...
  const existing = (
    await db.select().from(companies).where(eq(companies.name, name)).limit(1)
  )[0];
  if (existing) {
    await db
      .update(companies)
      .set({
        ...
        augustWindowStatus:
          input.augustWindowStatus ?? existing.augustWindowStatus,
      })
...
async function upsertVacancyFromResearch(
  ...
  const values = {
    department:
      input.department?.slice(0, 255) ||
      (/qa|quality|ipqa/i.test(input.title) ? "QA / IPQA" : null),
    location: input.location,
...
export async function seedResearchDirectory() {
  ...
  const [companyCsv, hrCsv] = await Promise.all([
    readFile(
      join(process.cwd(), "seed", "company_directory_2026_august.csv"),
      "utf8"
    ),
    readFile(
      join(process.cwd(), "seed", "hr_careers_directory_2026_august.csv"),
      "utf8"
    ),
  ]);
    ...
    candidates.set(
      ...
      existing
        ? {
            ...existing,
            primaryLocation:
              existing.primaryLocation ?? candidate.primaryLocation,
            careerUrl: existing.careerUrl ?? candidate.careerUrl,
            researchNote: existing.researchNote ?? candidate.researchNote,
            augustWindowStatus:
              existing.augustWindowStatus ?? candidate.augustWindowStatus,
            emails: Array.from(
              new Set([...existing.emails, ...candidate.emails])
            ),
            phones: Array.from(
              new Set([...existing.phones, ...candidate.phones])
            ),
  ...
  for (const row of directoryRows) {
    ...
    const sourceUrl = firstUrl(
      row["Official / Public Apply or Contact Source"] ?? ""
    );
    const eligibility = row["Eligibility / Salary"] ?? "";
    const name = canonicalCompanyName(rawName);
    addCandidate({
      name,
      primaryLocation: publicValue(locationInfo)
        ? locationInfo.slice(0, 255)
        : null,
    ...
    if (publicValue(title) && sourceUrl) {
      const location = publicValue(locationInfo)
        ? locationInfo.slice(0, 255)
        : "India — see source";
      const vacancy = {
        companyName: name,
        ...
        sourceUrl,
        eligibility,
        route: vacancyRoute(row["Walk-in or Direct Vacancy"] ?? title),
        twoYearMatch: /2[-\s–]*year|two-year/i.test(`${title} ${eligibility}`),
      };
  ...
  for (const row of hrRows) {
    ...
    addCandidate({
      ...
      emails: extractEmails(
        row["Public HR / Careers / Recruitment Email"] ?? ""
      ),
    ...
    if (
      publicValue(role) &&
      !/^not verified$/i.test(role.trim()) &&
      sourceUrl
    ) {
      const vacancy = {
        companyName: name,
        title: role,
        location: "India — see source",
        sourceUrl,
        eligibility,
        route: vacancyRoute(role),
        twoYearMatch: /2[-\s–]*year|two-year/i.test(`${role} ${eligibility}`),
      };
  ...
  const missingCompanies = Array.from(candidates.values())
    .filter(candidate => !companyByName.has(candidate.name))
    .map(candidate => ({
      name: candidate.name,
  ...
  for (let start = 0; start < missingCompanies.length; start += 100)
    await db
      .insert(companies)
      .values(missingCompanies.slice(start, start + 100));
  const allCompanies = await db.select().from(companies);
  const resolvedCompanies = new Map(
    allCompanies.map(company => [company.name, company])
  );

  const existingContacts = await db
    .select({
      companyId: companyContacts.companyId,
      contactType: companyContacts.contactType,
      contactValue: companyContacts.contactValue,
    })
    .from(companyContacts);
  const contactKeys = new Set(
    existingContacts.map(
      contact =>
        `${contact.companyId}|${contact.contactType}|${contact.contactValue}`
    )
  );
  ...
  candidates.forEach(candidate => {
    ...
    for (const email of candidate.emails) {
      const contactType = /\bhr\b|recruit|career/i.test(email)
        ? "hr"
        : ("careers" as const);
  ...
  for (let start = 0; start < newContacts.length; start += 200)
    await db
      .insert(companyContacts)
      .values(newContacts.slice(start, start + 200));

  const existingVacancies = await db
    .select({
  ...
  const vacancyKeys = new Set(
    existingVacancies.map(
      vacancy => `${vacancy.companyId}|${vacancy.title}|${vacancy.sourceUrl}`
    )
  );
  ...
  vacancyCandidates.forEach(candidate => {
    ...
    newVacancies.push({
      ...
      department: /qa|quality|ipqa/i.test(candidate.title) ? "QA / IPQA" : null,
      location: candidate.location,
      employmentRoute: candidate.route,
      walkInDateText: /august|aug|walk/i.test(candidate.title)
        ? candidate.title.slice(0, 255)
        : null,
      eligibility: candidate.eligibility || null,
      salaryText: /salary|ctc|₹|rs\.?/i.test(candidate.eligibility)
        ? candidate.eligibility.slice(0, 500)
        : null,
      sourceUrl: candidate.sourceUrl,
      status: "unverified",
      twoYearMatch: candidate.twoYearMatch,
      locationPriority: locationPriority(candidate.location),
      matchScore: scoreVacancy({
        title: candidate.title,
        eligibility: candidate.eligibility,
        location: candidate.location,
        route: candidate.route,
        twoYearHint: candidate.twoYearMatch,
      }),
    });
  });
  for (let start = 0; start < newVacancies.length; start += 100)
    await db.insert(vacancies).values(newVacancies.slice(start, start + 100));
...
export async function ensureCandidateProfile(user: User) {
  ...
  if (!existing) {
    await db.insert(candidateProfiles).values({
      ...
      preferredLocations:
        GUJARAT_PRIORITY_LOCATIONS.join(", ") + ", Bharuch, India-wide",
      cvVersion: "QA-OSD-v1",
    });
  }
  const profile = (
...
export async function updateCandidateProfile(
  userId: number,
  input: Partial<typeof candidateProfiles.$inferInsert>
) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db
...
export function rankVacanciesForProfile<T extends VacancyRankingInput>(
  rows: T[],
  profile: RankingProfile,
  limit = 80
) {
  return rows
    .map(row => {
      const location = inferVacancyLocation(
        row.vacancy.title,
        row.vacancy.location
      );
      const route = inferVacancyRoute(
        row.vacancy.title,
        row.vacancy.employmentRoute
      );
      return {
        ...row,
        vacancy: {
          ...
          matchScore: scoreVacancyForProfile(
            {
              ...
              route,
              twoYearHint:
                row.vacancy.twoYearMatch ||
                /1\s*[-–to]+\s*3|2\s*[-–to]+\s*\d|\b2\s*years?\b/i.test(
                  `${row.vacancy.title} ${row.vacancy.eligibility ?? ""}`
                ),
            },
            profile
          ),
    ...
    .sort(
      (left, right) =>
        right.vacancy.matchScore - left.vacancy.matchScore ||
        right.vacancy.locationPriority - left.vacancy.locationPriority
    )
    .slice(0, limit);
}
...
export async function getDashboardData(
  ...
  contacts.forEach(contact =>
    contactsByCompany.set(contact.companyId, [
      ...(contactsByCompany.get(contact.companyId) ?? []),
      contact,
    ])
  );
...
export async function getCompanyDirectory(query?: string) {
  ...
  const where = query?.trim()
    ? or(
        like(companies.name, `%${query.trim()}%`),
        like(companies.primaryLocation, `%${query.trim()}%`)
      )
    : undefined;
...
export async function exportDailyVacancyCsv() {
  ...
  return buildDailyVacancyCsv(
    rows.map(row => ({
      company: row.company.name.replace(/\s*\.+\s*$/g, ""),
      title: row.vacancy.title,
      department: row.vacancy.department,
      ...
      eligibility: row.vacancy.eligibility,
      salary: row.vacancy.salaryText,
      twoYearMatch: row.vacancy.twoYearMatch,
      matchScore: row.vacancy.matchScore,
      sourceUrl: row.vacancy.sourceUrl,
    }))
  );
}
...
export async function createVerifiedContactDraftsForSources(
  userId: number,
  sourceUrls: string[]
) {
  const db = await getDb();
  ...
  const uniqueSources = Array.from(
    new Set(sourceUrls.filter(sourceUrl => /^https?:\/\//i.test(sourceUrl)))
  );
  if (uniqueSources.length === 0)
    return { created: 0, skippedNoVerifiedContact: 0, skippedExistingDraft: 0 };
  const rows = await db
    .select({ vacancy: vacancies, company: companies })
    .from(vacancies)
    .innerJoin(companies, eq(vacancies.companyId, companies.id))
  ...
  await db
    ...
    .where(
      and(
        ...
        eq(applicationDrafts.userId, userId),
        eq(applicationDrafts.approvalStatus, "draft")
      )
    );
}
...
export async function markDraftSent(userId: number, draftId: number) {
  ...
  if (!canMarkDraftAsSent(draft.approvalStatus))
    throw new Error(
      "A draft must be explicitly approved before it can be marked as sent"
    );
  await db
    .update(applicationDrafts)
    .set({ approvalStatus: "sent", sentAt: new Date(), updatedAt: new Date() })
    .where(eq(applicationDrafts.id, draftId));
  if (draft.applicationId) {
    await db
      .update(applications)
      .set({
        status: applicationStatusAfterConfirmedSend(),
        dateApplied: new Date(),
        updatedAt: new Date(),
      })
      .where(
        and(
          eq(applications.id, draft.applicationId),
...
export async function createMonitoringRun(triggerType: "manual" | "scheduled") {
  ...
  const result = await db.insert(monitoringRuns).values({
    ...
    status: "completed",
    summary:
      "Imported and ranked source-cited August 16–29 research data. Live web-search execution is prepared for the scheduled agent handoff.",
...
export function formatGitHubPublishSummary(
  status: "pushed" | "no_changes" | "failed" | "dry_run",
  reportPath: string,
  detail?: string
) {
...
export async function recordGitHubPublishOutcome(
  status: "pushed" | "no_changes" | "failed" | "dry_run",
  reportPath: string,
  detail?: string
) {
...
export async function ingestScheduledVacancies(
  records: ScheduledVacancyInput[]
) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  let added = 0;
  for (const record of records) {
    ...
    const company = await upsertCompanyFromResearch(db, {
      ...
      researchNote:
        "Daily monitoring agent: public source-cited record. Confirm before applying.",
      augustWindowStatus: record.title,
    });
    await upsertPublicContacts(
      db,
      company.id,
      record.publicEmail ?? "",
      record.publicPhone ?? "",
    ...
    const created = await upsertVacancyFromResearch(db, {
      ...
      eligibility: [record.eligibility, record.salaryText]
        .filter(Boolean)
        .join(" "),
...
export async function getMonitoringSchedule(userId: number) {
  ...
  return (
    await db
      ...
      .where(eq(monitoringSchedules.userId, userId))
      .limit(1)
  )[0];
}
...
export async function getMonitoringScheduleByTaskUid(taskUid: string) {
  ...
  return (
    await db
      .select()
      .from(monitoringSchedules)
      .where(eq(monitoringSchedules.scheduleCronTaskUid, taskUid))
      .limit(1)
  )[0];
}

export async function saveMonitoringScheduleTask(
  userId: number,
  taskUid: string
) {
  const db = await getDb();
  ...
  await db
    ...
    .set({ scheduleCronTaskUid: taskUid, enabled: true, updatedAt: new Date() })
    .where(eq(monitoringSchedules.userId, userId));
