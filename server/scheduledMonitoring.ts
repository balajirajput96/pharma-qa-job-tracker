...
import { createMonitoringRun, getMonitoringScheduleByTaskUid, ingestScheduledVacancies, type ScheduledVacancyInput } from "./db";
...
export async function runScheduledMonitoring(req: Request, res: Response) {
  try {
    ...
    const records = Array.isArray(req.body?.vacancies) ? req.body.vacancies as ScheduledVacancyInput[] : [];
    ...
    return res.json({ ok: true, runId, newVacancies: added, ownerNotified: notified });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return res.status(500).json({ error: message, timestamp: new Date().toISOString(), context: { path: "/api/scheduled/daily-monitoring" } });
  }
}
