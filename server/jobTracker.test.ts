...
import {
  ...
  applicationStatusAfterConfirmedSend,
  buildDailyVacancyCsv,
  canMarkDraftAsSent,
  formatGitHubPublishSummary,
  inferVacancyLocation,
  inferVacancyRoute,
  locationPriority,
  rankVacanciesForProfile,
  scoreVacancy,
  scoreVacancyForProfile,
} from "./db";

describe("job matching priority", () => {
  it("prioritizes Vadodara over India-wide locations", () => {
    expect(locationPriority("Vadodara, Gujarat")).toBeGreaterThan(
      locationPriority("Hyderabad, Telangana")
    );
  });

  it("recovers a Gujarat location and walk-in route from a sourced vacancy title", () => {
    const title =
      "Walk-In Interview on 18 August 2026 for Production (OSD), Quality Assurance (IPQA & QMS) at Gota, Ahmedabad";
    expect(inferVacancyLocation(title, "India — see source")).toBe(
      "Ahmedabad, Gujarat"
    );
  ...
  it("uses saved candidate preferences to boost a matching Gujarat role", () => {
    const profile = {
      ...
      preferredLocations:
        "Vadodara, Ahmedabad, Halol, Sanand, Ankleshwar, India-wide",
  ...
  it("applies candidate-specific scoring before reducing the dashboard to its result limit", () => {
    const profile = {
      experienceYears: 2,
      skills: "IPQA, Tablet Compression, OSD",
      preferredLocations: "Vadodara, Gujarat",
    } as Parameters<typeof rankVacanciesForProfile>[1];
    const genericRows = Array.from({ length: 80 }, (_, index) => ({
      ...
      vacancy: {
        ...
        eligibility: "5 years experience",
        location: "India",
        employmentRoute: "direct" as const,
        twoYearMatch: false,
        matchScore: 90,
        locationPriority: 0,
      },
    ...
    const exactMatch = {
      ...
      vacancy: {
        ...
        location: "Vadodara, Gujarat",
        employmentRoute: "walk_in" as const,
        twoYearMatch: true,
        matchScore: 40,
        locationPriority: 0,
      },
    };
    const ranked = rankVacanciesForProfile(
      [...genericRows, exactMatch],
      profile,
      80
    );
...
describe("approval-gated application drafts", () => {
  it("keeps the required subject exact for every generated draft", () => {
    expect(APPLICATION_SUBJECT).toBe(
  ...
  it("formats tracker-visible GitHub publish outcomes for success and failure", () => {
    expect(
      formatGitHubPublishSummary("pushed", "reports/report.csv")
    ).toContain("pushed: reports/report.csv");
    expect(
      formatGitHubPublishSummary(
        "failed",
        "reports/report.csv",
    ...
    ).toContain("remote rejected push");
  });
});
