...
export const appRouter = router({
  ...
  jobTracker: router({
    bootstrapResearch: protectedProcedure.mutation(async () =>
      seedResearchDirectory()
    ),
    ...
    directory: protectedProcedure
      .input(z.object({ query: z.string().max(160).optional() }))
      .query(async ({ input }) => getCompanyDirectory(input.query)),
    profile: protectedProcedure.query(async ({ ctx }) =>
      ensureCandidateProfile(ctx.user)
    ),
    saveProfile: protectedProcedure
      .input(profileInput)
      .mutation(async ({ ctx, input }) =>
        updateCandidateProfile(ctx.user.id, input)
      ),
    applications: protectedProcedure.query(async ({ ctx }) =>
      getApplicationsForUser(ctx.user.id)
    ),
    ...
    drafts: protectedProcedure.query(async ({ ctx }) =>
      getDraftsForUser(ctx.user.id)
    ),
    createDraft: protectedProcedure
      .input(z.object({ vacancyId: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => ({
        draftId: await createApplicationDraft(ctx.user, input.vacancyId),
      })),
    ...
    markDraftSent: protectedProcedure
      .input(z.object({ draftId: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => {
        await markDraftSent(ctx.user.id, input.draftId);
        return { success: true };
      }),
    runMonitoring: protectedProcedure.mutation(async () => ({
      runId: await createMonitoringRun("manual"),
    })),
    monitoringSchedule: protectedProcedure.query(async ({ ctx }) =>
      getMonitoringSchedule(ctx.user.id)
    ),
    activateDailySchedule: protectedProcedure.mutation(async ({ ctx }) => {
      if (process.env.NODE_ENV !== "production")
        throw new Error(
          "Publish this site before activating the daily schedule."
        );
      ...
      const specification = {
        ...
        payload: {},
        description:
          "Daily 8:30 AM IST Pharma QA/IPQA/OSD monitoring and owner notification",
      };
      if (schedule.scheduleCronTaskUid) {
        await updateHeartbeatJob(
          schedule.scheduleCronTaskUid,
          { ...specification, enable: true },
          sessionToken
        );
        return { activated: true, existing: true };
      }
      const job = await createHeartbeatJob(
        { name: `pharma-daily-monitoring-${ctx.user.id}`, ...specification },
        sessionToken
      );
